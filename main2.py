...
from scripts.gstreamer_utils import create_gstreamer_capture, create_gstreamer_writer, create_opencv_capture, create_opencv_writer, GSTREAMER_AVAILABLE
import os
...
def load_config():
    ...
    try:
        ...
        print("⚠️  config/params.yaml not found, using default configuration")
        return {
            'gstreamer': {
                'use_hw_accel': False,
...
def create_mediamtx_stream_writer():
    """Create a GStreamer writer that streams to MediaMTX"""
    if not GSTREAMER_AVAILABLE:
        print("⚠️  GStreamer not available, falling back to file output")
        return cv2.VideoWriter('output/detection_output.mp4', 
                              cv2.VideoWriter_fourcc(*'mp4v'), 20, (1920, 1080))
    
    # GStreamer pipeline to stream to MediaMTX
    pipeline = (
        'appsrc ! videoconvert '
        '! video/x-raw,format=I420 '
        '! x264enc speed-preset=ultrafast bitrate=1000 key-int-max=40 '
        '! video/x-h264,profile=baseline '
        '! rtspclientsink location=rtsp://localhost:8888/detection_output'
    )
    
    try:
        writer = cv2.VideoWriter(pipeline, cv2.CAP_GSTREAMER, 0, 20, (1920, 1080), True)
        if writer.isOpened():
            print("✅ MediaMTX stream writer created successfully")
            return writer
        else:
            print("⚠️  Failed to create MediaMTX stream writer, falling back to file output")
            return cv2.VideoWriter('output/detection_output.mp4', 
                                  cv2.VideoWriter_fourcc(*'mp4v'), 20, (1920, 1080))
    except Exception as e:
        print(f"⚠️  Error creating MediaMTX stream writer: {e}")
        return cv2.VideoWriter('output/detection_output.mp4', 
                              cv2.VideoWriter_fourcc(*'mp4v'), 20, (1920, 1080))
...
if __name__ == "__main__":
    ...
    gstreamer_config = config_data.get("gstreamer", {})
    use_hw_accel = gstreamer_config.get("use_hw_accel", True)
    gstreamer_width = gstreamer_config.get("width", 1920)
    gstreamer_height = gstreamer_config.get("height", 1080)
    gstreamer_fps = gstreamer_config.get("fps", 30)
    
    # Use GStreamer input source if specified, otherwise fall back to original video path
    input_source = gstreamer_config.get("input_source", video_path)
    if not os.path.isabs(input_source):
        input_source = os.path.join(dir_path, input_source)
    ...
    # Create video capture (GStreamer or OpenCV fallback)
    print(f"Initializing video capture with source: {input_source}")
    
    if GSTREAMER_AVAILABLE:
        try:
            print("Using GStreamer capture with hardware acceleration")
            cap = create_gstreamer_capture(
                input_source,
                width=gstreamer_width,
                height=gstreamer_height,
                fps=gstreamer_fps,
            ...
            if not cap.start():
                print("Error: Could not start GStreamer pipeline. Falling back to OpenCV.")
                cap = create_opencv_capture(input_source)
        except Exception as e:
            print(f"Error initializing GStreamer: {e}. Falling back to OpenCV.")
            cap = create_opencv_capture(input_source)
    else:
        print("GStreamer not available. Using OpenCV capture.")
        cap = create_opencv_capture(input_source)
    ...
    # Video parameters (use GStreamer dimensions)
    ...
    # Video writer with MediaMTX integration
    if generate_video:
        print("📺 Creating MediaMTX stream writer...")
        video_writer = create_mediamtx_stream_writer()
        
        # Also create local file writer as backup
        local_writer = create_opencv_writer(output_path, roi_width, roi_height, fps)
        print("📁 Local file writer created as backup")
    ...
    # Check if we're using GStreamer capture
    is_gstreamer = hasattr(cap, 'running') and cap.running
    ...
    print("📺 MediaMTX stream available at: rtsp://localhost:8888/detection_output")
    print("🌐 WebRTC stream available at: http://localhost:8888/webrtc_stream")
    print("📊 API available at: http://localhost:9997")
    print("📈 Metrics available at: http://localhost:9998")
